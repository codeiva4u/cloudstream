...
class InAppUpdater {
    companion object {
        ...
        private fun openApk(context: Activity, uri: Uri) {
            try {
                uri.path?.let {
                    ...
                    // Perform post-update operations after installation
                    performPostUpdateOperations(context)
                }
            } catch (e: Exception) {
                logError(e)
            }
        ...
        /**
         * Performs post-update operations such as cleaning up cache and temporary files.
         */
        private fun performPostUpdateOperations(context: Activity) {
            try {
                // 1. Clean up cache files
                cleanupCacheFiles(context)
                // 2. Remove temporary files
                removeTemporaryFiles(context)
                // 3. Log the completion of post-update operations
                Log.d(LOG_TAG, "Post-update operations completed successfully.")
            } catch (e: Exception) {
                Log.e(LOG_TAG, "Failed to perform post-update operations: ${e.message}")
            }
        }
         ...
         */
        private fun cleanupCacheFiles(context: Activity) {
            try {
                ...
                if (cacheDir.exists()) {
                    cacheDir.listFiles()?.forEach { file ->
                        if (file.isFile) {
                            ...
                            if (deleted) {
                                Log.d(LOG_TAG, "Deleted cache file: ${file.absolutePath}")
                            } else {
                                Log.e(LOG_TAG, "Failed to delete cache file: ${file.absolutePath}")
                            }
                        }
                    }
                }
            } catch (e: Exception) {
                Log.e(LOG_TAG, "Failed to clean up cache files: ${e.message}")
            }
        }
         ...
         */
        private fun removeTemporaryFiles(context: Activity) {
            try {
                ...
                if (tempDir.exists()) {
                    tempDir.listFiles()?.forEach { file ->
                        if (file.isFile) {
                            val deleted = file.delete()
                            if (deleted) {
                                Log.d(LOG_TAG, "Deleted temporary file: ${file.absolutePath}")
                            } else {
                                Log.e(LOG_TAG, "Failed to delete temporary file: ${file.absolutePath}")
                            }
                        }
                    }
                }
            } catch (e: Exception) {
                Log.e(LOG_TAG, "Failed to remove temporary files: ${e.message}")
            }
        }
